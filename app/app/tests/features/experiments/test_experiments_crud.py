...
from app.features.experiments.crud import ExperimentUpdateRepo, repo as experiment_repo
...
class TestExperimentRepo:
    def test_update(self, db: Session, some_experiment: Experiment):
        update = ExperimentUpdateRepo(
            ...
            history={'train_loss': [1349.7373046875, 544.4139404296875, 546.408447265625, 514.2393188476562, 495.88104248046875], 'epoch': [0, 1, 2, 3, 4]})

        db_obj = db.query(ExperimentEntity).filter(ExperimentEntity.id == some_experiment.id).first()
