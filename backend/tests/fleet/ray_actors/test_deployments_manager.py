...
class TestDeploymentsManager:
    ...
    async def test_add_deployment(self, some_deployment: Deployment):
        ...
        deployment: Deployment = await manager.add_deployment.remote(
            some_deployment
        )
    ...
    async def test_make_prediction(self, some_deployment: Deployment):
        ...
        prediction: Dict[
            str, torch.Tensor
        ] = await manager.make_prediction.remote(some_deployment.id, data)
        ...
        assert prediction["tpsa"].shape == (
            3,
        ), "'tpsa' column should have shape (3,)"
