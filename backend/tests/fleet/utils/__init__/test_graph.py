...
def test_make_graph_from_forward_args():
    ...
    nodes = [
        TestAnyNode(name="a", forward_args={"inputa": "$b.output"}),
        TestAnyNode(name="b", forward_args={"inputb": ["$c.a", "$d.b"]}),
        TestAnyNode(name="c", forward_args={"inputc": "$c"}),
        TestAnyNode(name="d", forward_args={"inputd": "$d"}),
    ]
