...
def test_update_deployment(
    ...
    r = client.put(
        f"{get_app_settings('server').api_v1_str}/deployments/{deployment_fixture.id}",
...
def test_delete_deployment(
    ...
    with create_temporary_deployment(
        ...
        r = client.delete(
            f"{get_app_settings('server').api_v1_str}/deployments/{some_deployment.id}",
            headers=normal_user_token_headers,
        )
        assert r.status_code == 200
        payload = r.json()
...
def test_create_permission(
    ...
    r = client.post(
        f"{get_app_settings('server').api_v1_str}/deployments/create-permission",
...
def test_delete_permission(
    ...
    r = client.post(
        f"{get_app_settings('server').api_v1_str}/deployments/delete-permission",
...
def test_get_public_deployment(
    ...
    with create_temporary_deployment(
        ...
        r = client.get(
            f"{get_app_settings('server').api_v1_str}/deployments/public/{token}"
        )
...
def test_post_make_prediction(
    ...
    r = client.post(
        f"{get_app_settings('server').api_v1_str}/deployments/{some_deployment.id}/predict",
