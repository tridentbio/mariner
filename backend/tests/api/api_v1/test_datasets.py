...
def test_put_datasets(
    ...
    r = client.put(
        f"{get_app_settings('server').host}/api/v1/datasets/{some_dataset.id}",
        json={
            "name": new_name,
...
def test_delete_datasets(
    ...
    r = client.delete(
        f"{get_app_settings('server').host}/api/v1/datasets/{dataset.id}",
        headers=normal_user_token_headers,
    )
    assert r.status_code == status.HTTP_200_OK
...
def test_get_csv_metadata(
    ...
    with open("tests/data/csv/Lipophilicity.csv", "rb") as f:
        res = client.post(
            f"{get_app_settings('server').host}/api/v1/datasets/csv-metadata",
            files={"file": ("dataset.csv", f.read())},
            headers=normal_user_token_headers,
        )
        assert res.status_code == status.HTTP_200_OK
...
def test_download_dataset(
    ...
    res = client.get(
        f"{get_app_settings('server').host}/api/v1/datasets/{some_dataset_without_process.id}/{route}",
        headers=normal_user_token_headers,
    )
