...
def test_post_models_success(
    ...
    model = mock_model()
    res = client.post(
        f"{get_app_settings('server').api_v1_str}/models/",
        json=model.dict(),
...
def test_get_model_options(
    ...
    res = client.get(
        f"{get_app_settings('server').api_v1_str}/models/options",
        headers=normal_user_token_headers,
    )
    assert res.status_code == HTTP_200_OK
    payload = res.json()
...
def test_delete_model(
    ...
    res = client.delete(
        f"{get_app_settings('server').api_v1_str}/models/{model.id}",
        headers=normal_user_token_headers,
    )
...
def test_post_predict(
    ...
    route = f"{get_app_settings('server').api_v1_str}/models/{model_version}/predict"
    res = client.post(
        route,
        json={
            "smiles": [
...
def test_get_model_version(
    ...
    res = client.get(
        f"{get_app_settings('server').api_v1_str}/models/{some_model.id}/",
        headers=normal_user_token_headers,
    )
    assert res.status_code == 200
    body = res.json()
...
def test_post_check_config_good_model(
    ...
    res = client.post(
        f"{get_app_settings('server').api_v1_str}/models/check-config",
        headers=normal_user_token_headers,
        json=payload.dict(),
    )
...
def test_get_name_suggestion(
    ...
    res = client.get(
        f"{get_app_settings('server').api_v1_str}/models/name-suggestion",
        headers=normal_user_token_headers,
    )
    assert res.status_code == 200
    body = res.json()
