...
/**
 * Interface needed for ModelEditorV2 Context.
 * Each model editor must have a different `ModelEditorContextProvider`
 *
 */
export interface IModelEditorContext
   ...
   * (through `ModelEditor` methods)
   */
  schema?: ModelSchema;

  /**
   * Makes a hard change to the schema (prefer using `addComponent`, `editComponent`,
   * `deleteComponents` and other methods from `ModelEditor` ). It's only use is to
