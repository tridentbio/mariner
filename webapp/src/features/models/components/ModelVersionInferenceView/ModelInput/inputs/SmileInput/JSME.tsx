/**
 * For now, this is a copy of the TCW JSME Input repository.
 * When it is possible to npm install TCW, we can do that, and import
...
const JSME: React.FC<JSMEProps> = (props): JSX.Element => {
  const [jsmeIdState, _setJsmeIdState] = useState(
    'jsme' + getRandomInt(1, 100000)
  );
  const [jsmeLoadedState, setJsmeLoadedState] = useState(false);
  const [jsmeAppletState, setJsmeAppletState] = useState<any>();
  const [currentValue, setCurrentValue] = useState(props.smiles);

  const handleChange = (jsmeEvent: any) => {
    let newSmiles = jsmeEvent.src.smiles();
    setCurrentValue(newSmiles);
    if (props.onChange) {
      props.onChange(newSmiles);
    }
  };

  const handleJsmeLoad = (reset = false) => {
    const optionObject = {
      options: props.options ? props.options : '',
      ...props.altOptions,
    };

    let jsmeApplet: any = new (window as any).JSApplet.JSME(
      jsmeIdState,
      props.width,
      props.height,
      optionObject
    ...
    jsmeApplet.readGenericMolecularInput(reset ? '' : props.smiles);
  ...
  const setup = (src: string) => {
    ...
    (window as any).jsmeOnLoad = () => {
      Object.values(jsmeCallbacks).forEach((f: any) => f());
    };
  };
  ...
  const reset = useCallback(() => {
    const div = document.getElementById(jsmeIdState);
    div?.innerHTML && (div.innerHTML = '');
    handleJsmeLoad(true);
  }, [jsmeIdState]);

  useEffect(() => {
    if (!props.smiles && currentValue) reset();
  }, [props.smiles]);
